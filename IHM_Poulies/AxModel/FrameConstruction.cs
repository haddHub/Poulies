...
namespace AxModel
{
    ...
    public class FrameConstruction
    {
        ...
        public static aXdataModel DeconstructFrame(byte[] encodedFrame)
        {
            ...
            switch (decodedFrame[2])
            {
                case ((byte)FrameHeaders.Error):
                    ...
                    return new ErrorDataModel((int)ConcatS((byte)decodedFrame[6], (byte)decodedFrame[7]));
                ...
                case ((byte)FrameHeaders.Force2):   // Force de référence
                    if (decodedFrame.Length != 12)
                    {
                        throw new Exception("Invalid number of bytes for frame header.");
                    }
                ...
                case ((byte)FrameHeaders.ForceRap):   // Force de référence
                    if (decodedFrame.Length != 12)
                    {
                        throw new Exception("Invalid number of bytes for frame header.");
                    }
                    return new ForceRapDataModel((int)ConcatS((byte)decodedFrame[4], (byte)decodedFrame[5]), (int)ConcatS((byte)decodedFrame[6], (byte)decodedFrame[7]));
                case ((byte)FrameHeaders.ForceRap2):   // Force de référence
                    if (decodedFrame.Length != 12)
                    {
                        throw new Exception("Invalid number of bytes for frame header.");
                    }
                    return new ForceRap2DataModel((int)ConcatS((byte)decodedFrame[4], (byte)decodedFrame[5]), (int)ConcatS((byte)decodedFrame[6], (byte)decodedFrame[7]));
                case ((byte)FrameHeaders.AcosT):   // Force de référence
                    if (decodedFrame.Length != 12)
                    {
                        throw new Exception("Invalid number of bytes for frame header.");
                    }
                    return new AcosTDataModel((int)ConcatS((byte)decodedFrame[4], (byte)decodedFrame[5]), (int)ConcatS((byte)decodedFrame[6], (byte)decodedFrame[7]));
                case ((byte)FrameHeaders.Couple):   // Couple de référence
                    if (decodedFrame.Length != 12)
                    {
                        throw new Exception("Invalid number of bytes for frame header.");
                    }
                    return new CoupleDataModel((int)ConcatS((byte)decodedFrame[4], (byte)decodedFrame[5]), (int)ConcatS((byte)decodedFrame[6], (byte)decodedFrame[7]));
                ...
                case ((byte)FrameHeaders.Vitesse):
                    ...
                    return new VitesseDataModel((int)ConcatS(decodedFrame[4], decodedFrame[5]), (int)ConcatS(decodedFrame[6], decodedFrame[7])); // TODO : changer la forceREf ????
                case ((byte)FrameHeaders.Vitesse2):
                    if (decodedFrame.Length != 12)
                    {
                        throw new Exception("Invalid number of bytes for frame header.");
                    }
                    return new Vitesse2DataModel((int)ConcatS(decodedFrame[4], decodedFrame[5]), (int)ConcatS(decodedFrame[6], decodedFrame[7])); // TODO : changer la forceREf ????
                ...
                case ((byte)FrameHeaders.ACK_mod_game):
                    ...
                    return new ACKDataModel(decodedFrame);
                case ((byte)FrameHeaders.ACK_KintCint):
                    if (decodedFrame.Length != 12)
                    {
                        throw new Exception("Invalid number of bytes for frame header.");
                    }
                    return new FrameExerciceDataModel(ConfigAddresses.KintCint, decodedFrame[4], decodedFrame[5], decodedFrame[6], decodedFrame[7]);
                case ((byte)FrameHeaders.ACK_VsJssParamInt):
                    if (decodedFrame.Length != 12)
                    {
                        throw new Exception("Invalid number of bytes for frame header.");
                    }
                    return new FrameExerciceDataModel(ConfigAddresses.VsJssParamInt, decodedFrame[4], decodedFrame[5], decodedFrame[6], decodedFrame[7]);
                case ((byte)FrameHeaders.ACK_Var1Var2):
                    if (decodedFrame.Length != 12)
                    {
                        throw new Exception("Invalid number of bytes for frame header.");
                    }
                    return new FrameConfigDataModel(ConfigAddresses.Var1Var2, Concat(decodedFrame[4], decodedFrame[5]), Concat(decodedFrame[6], decodedFrame[7]));
                case ((byte)FrameHeaders.ACK_Var3Var4):
                    if (decodedFrame.Length != 12)
                    {
                        throw new Exception("Invalid number of bytes for frame header.");
                    }
                    return new FrameConfigDataModel(ConfigAddresses.Var3Var4, Concat(decodedFrame[4], decodedFrame[5]), Concat(decodedFrame[6], decodedFrame[7]));
                case ((byte)FrameHeaders.ACK_Regulateur):
                    if (decodedFrame.Length != 12)
                    {
                        throw new Exception("Invalid number of bytes for frame header.");
                    }
                    return new FrameExerciceDataModel(ConfigAddresses.Regulateur, decodedFrame[4], decodedFrame[5], decodedFrame[6], decodedFrame[7]);
                ...
                case ((byte)FrameHeaders.ACK_KlatClong):
                    ...
                    return new FrameExerciceDataModel(ConfigAddresses.KlatClong, decodedFrame[4], decodedFrame[5], decodedFrame[6], decodedFrame[7]);
                ...
                case ((byte)FrameHeaders.ACK_ActifPassif):
                    ...
                    return new FrameExerciceDataModel(ConfigAddresses.ActifPassif, decodedFrame[4], decodedFrame[5], decodedFrame[6], decodedFrame[7]);
        ...
        /// MSB and LSB concatenated to create a short.
        /// </returns>
        private static ushort Concat(byte MSB, byte LSB)
        {
            return (ushort)((ushort)((ushort)MSB << 8) | (ushort)LSB);
        }
        #endregion
    }
}
